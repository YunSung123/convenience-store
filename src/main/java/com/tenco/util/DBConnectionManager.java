...
// DB 접근해서 Connection 객체를 생성하는 역할의 클래스 이다.
...
public class DBConnectionManager {
    ...
    // static 초기화 블럭 : 클래스가 메모리에 로딩될 떄 단 1번만 실행하는 녀석 보장
    static {
        ...
        config.setConnectionTimeout(5000); // 커넥션 할당 대기 시간 제한 (5초)
        config.setIdleTimeout(600000); // 유후 거넥션 유지 시간 (10분)

        dataSource = new HikariDataSource(config);
    }

    // 사용자가 호출하는 메서드 생성 - 풀에서 커넥션 객체 하나를 빌려 온다.
